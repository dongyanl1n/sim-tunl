...
class IntervalTiming(object):
    '''
    Interval timing task in triangular arena. After initiating the trial, the agent experiences either short or long
    delay, then initiates the choice phase. If short delay, then poke left to get reward. If long, then poke right.
    '''

    def __init__(self, len_delay_short, len_delay_long, len_edge, rwd, inc_rwd, step_rwd, poke_rwd, rng_seed=1234):
        """
        ...
        assert len_delay_short < len_delay_long
        ...
        self.dist_to_init = None
        self.done = False
        self.len_delay = None  # Going to be either short or long
        self.len_delay_short = len_delay_short
        self.len_delay_long = len_delay_long
        self.sample = "undefined"
        self.rwd = rwd
        self.inc_rwd = inc_rwd
        self.step_rwd = step_rwd
    ...
    def reset(self):
        ...
        self.sample = "undefined"
        if not self.correction_trial:
            self.len_delay = random.choices((self.len_delay_short, self.len_delay_long))[0]
        ...
        if action in [0, 1, 2, 3, 4]:  # movement
            ...
            if np.sum(self.observation[self.current_loc]) > 255:  # currently at a signal location
                if self.current_loc == self.initiation_loc:  # currently at initiation signal location
                    ...
                    self.observation[self.current_loc] -= self.color[
                        "touchscreen_loc"]  # turn off touchscreen signal
                    if self.sample == self.len_delay_long:  # incorrect
                        self.reward = self.inc_rwd
                        self.done = True
                        self.correction_trial = True
                    elif self.sample == self.len_delay_short:  # correct
                        self.reward = self.rwd
                        self.done = True
                        self.correction_trial = False
                elif self.current_loc == self.right_loc:  # currently at the right touchscreen
                    self.observation[self.current_loc] -= self.color[
                        "touchscreen_loc"]  # turn off touchscreen signal
                    if self.sample == self.len_delay_short:  # incorrect
                        self.reward = self.inc_rwd
                        self.done = True
                        self.correction_trial = True
                    elif self.sample == self.len_delay_long:  # correct
                        self.reward = self.rwd
                        self.done = True
                        self.correction_trial = False
            else:
...
class Tunl_loc_fix(object):
    def __init__(self, len_delay, len_edge, rwd, inc_rwd, step_rwd, poke_rwd, rng_seed=1234):
        """
        After receiving the sample, the agent will be teleported to the location equi-distant from both L and R ports
        on the long edge, and stay there throughout the delay until the initiation signal for choice phase comes back on
    ...
    def step(self, action):
        ...
        assert self.action_space.contains(action)
        if action in [0, 1, 2, 3, 4]:  # movement
            if not self.indelay:  # Location-fixed during delay
                self.reward = self.step_rwd  # lightly punish all step actions except during delay
                self.nav_reward += self.reward
                next_loc = tuple(self.current_loc + self.directions[action])
                if not self.walls[next_loc]:  # if next location is not walls
            ...
            if np.sum(self.observation[self.current_loc]) > 255:  # currently at a signal location
                if self.current_loc == self.initiation_loc:  # currently at initiation signal location
                    ...
                    if self.sample == "undefined":  # enter delay
                        ...
                        self.reward = self.poke_rwd
                        self.nav_reward += self.reward
                        self.observation[self.current_loc] -= self.color["current_loc"]  # leave current loc
                        self.current_loc = self.delay_loc
                        self.observation[self.delay_loc] += self.color["current_loc"]  # teleported to delay_loc
                    elif self.sample == "L": # poked incorrectly at match location
                        self.reward = self.inc_rwd
                        self.correction_trial = True  # set the flag so the next trial gets the same sample_loc
                        self.done = True
...
class Tunl_nomem_loc_fix(object):
    def __init__(self, len_delay, len_edge, rwd, step_rwd, poke_rwd, rng_seed=1234):
        """
        :param len_delay: int
        :param len_edge: odd int -- length of long edge in triangle (minimum = 5)
...
class Tunl_nomem_vd_loc_fix(object):
    '''
    No-memory-load TUNL with varying delay. For each trial, the length of delay is randomly sampled from len_delays
    with probability len_delays_p
    '''
            ...
            if np.sum(self.observation[self.current_loc]) > 255:  # currently at a signal location
                if self.current_loc == self.initiation_loc:  # currently at initiation signal location
                    ...
                    if self.sample == "undefined":  # enter delay
                        ...
                        self.reward = self.poke_rwd
                        self.nav_reward += self.reward
                    elif self.sample == "L": # poked correctly at the match location
                        self.reward = self.rwd
                        self.correction_trial = False  # reset correction_trial Flag
                        self.done = True
                    elif self.sample == "R":  # poked incorrectly at nonmatch location
                        self.reward = self.inc_rwd
                        self.correction_trial = True  # set the flag so the next trial gets the same sample_loc
                        self.done = True
                elif self.current_loc == self.right_loc:  # currently at the right touchscreen
                    ...
                    if self.sample == "undefined":  # enter delay
                        ...
                        self.reward = self.poke_rwd
                        self.nav_reward += self.reward
                    elif self.sample == "R":  # poked correctly at match location
                        self.reward = self.rwd
                        self.correction_trial = False  # reset correction_trial Flag
                        self.done = True
                    elif self.sample == "L":  # poked incorrectly at nonmatch location
                        self.reward = self.inc_rwd
                        self.correction_trial = True  # set the flag so the next trial gets the same sample_loc
                        self.done = True
