...
def finish_trial_l2(model, discount_factor, optimizer, resp, lambda_l2, **kwargs):
    '''
    Finishes a given training trial and backpropagates.
    Here, total_loss includes l2_loss (ridge) regularization,
    with scaling factor lambda_l2, of hidden state of
    the LSTM/GRU layer or the cell output of linear layer.
    Arguments:
    - model: AC_net
    - discount_factor: float
    - optimizer
    - resp: array of shape (number of timesteps in this episode x n_neurons)
    - lambda_l2: float
    Returns:
    - p_loss: sum of policy losses at each time step in this trial
    - v_loss: sum of value losses at each time step in this trial
    - l2_loss: sum of square of hidden states / cell states at all time steps in this trial
    '''
    ...
    for (log_prob, value), r in zip(saved_actions, returns_):
        ...
        #   return policy_losses, value_losses
    optimizer.zero_grad() # clear gradient
    p_loss = (torch.cat(policy_losses).sum()) # Monte-Carlo, sum of policy loss at each time step in the episode
    v_loss = (torch.cat(value_losses).sum()) # Monte-Carlo, sum of value loss at each time step in the episode
    #l1_loss = np.sum([np.abs(activ) for activ in resp]) ######
    l2_loss = np.sum([(activ)**2 for activ in resp]) / len(resp)  #ref: Cueva ICLR
    total_loss = p_loss + v_loss + lambda_l2*l2_loss ######
    total_loss.backward(retain_graph=True) # calculate gradient
...
def tbptt(model, discount_factor, optimizer, **kwargs):
    """
    Perform truncated BPTT on the last k1=k2 timesteps. (Sutskever 2013: "It processes the sequence one timestep at a
    time, and every k1 timesteps, it runs BPTT for k2 timesteps, so a parameter update can be cheap if k2 is small.")
    """
    ...
    total_loss.backward(retain_graph=False)  # calculate gradient  # False or True??
    optimizer.step()

    del model.rewards[:]
    del model.saved_actions[:]
    model.reinit_hid()  # THIS IS THE ONLY CHANGE SO FAR
